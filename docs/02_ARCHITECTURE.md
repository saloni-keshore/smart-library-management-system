...
3. Route opens a connection via `get_connection()`, or calls into a `database/*_queries.py` module (some blueprints use raw SQL inline instead, see [05_ROUTES_REFERENCE.md](05_ROUTES_REFERENCE.md) for which) — as of 2026-07-23, `routes/auth.py` (ADR-16), `routes/setting.py`'s `security_settings()` password branch only (ADR-17), `routes/enquiries.py` (ADR-18), and `routes/student.py` (ADR-19) are the exceptions: they call `database/supabase_client.py`'s `get_supabase_client()` instead, since `admins` reads/writes for login/register/forgot-password, Settings' password change, all of Enquiries, and all of Students now go to Supabase (PostgreSQL), not SQLite. Both `routes/enquiries.py` and `routes/student.py` also still call `get_connection()` — not for their own reads, but to keep a SQLite mirror in sync for `routes/membership.py`/`routes/payment.py`/`routes/dashboard.py`/etc.'s benefit (all still unmigrated, and each still JOINs `students`/`enquiries` directly in SQL).
...
- **Two data-access styles coexist:** some blueprints (`dashboard`, `notification`, part of `membership`/`membership_distribution`) write raw SQL directly in the route function; others (`cashbook`, `business_intelligence`, `setting`) delegate to a dedicated `database/*_queries.py` module. See [09_DEPENDENCY_MAP.md](09_DEPENDENCY_MAP.md). `auth` is a third, newer style as of 2026-07-23: it queries Supabase's PostgREST client directly in the route function (no query module yet either), the same "inline" shape as the first group, just against a different backend — see ADR-16. `setting` picked up a small slice of this same third style the same day (ADR-17): `security_settings()`'s password-change branch calls `get_supabase_client()` directly, inline, while every other function in that file still delegates to a `database/*_queries.py` module as before. `enquiries` and `student` both moved fully into the third style the same day too (ADR-18/ADR-19) — every function calls `get_supabase_client()` directly, inline, though both also fall back to raw SQL (`get_connection()`) to keep a SQLite mirror in sync for the other's/unmigrated modules' benefit and (for `student`) to read `memberships`/`payments`, which stay SQLite.
