...
3. Route opens a connection via `get_connection()`, or calls into a `database/*_queries.py` module (some blueprints use raw SQL inline instead, see [05_ROUTES_REFERENCE.md](05_ROUTES_REFERENCE.md) for which).
...
- **Two data-access styles coexist:** some blueprints (`auth`, `dashboard`, `student`, `enquiries`, `notification`, part of `membership`/`membership_distribution`) write raw SQL directly in the route function; others (`cashbook`, `business_intelligence`, `setting`) delegate to a dedicated `database/*_queries.py` module. See [09_DEPENDENCY_MAP.md](09_DEPENDENCY_MAP.md).
