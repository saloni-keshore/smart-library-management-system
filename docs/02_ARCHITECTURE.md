...
3. Route opens a connection via `get_connection()`, or calls into a `database/*_queries.py` module (some blueprints use raw SQL inline instead, see [05_ROUTES_REFERENCE.md](05_ROUTES_REFERENCE.md) for which) — as of 2026-07-23, `routes/auth.py` (ADR-16), `routes/setting.py`'s `security_settings()` password branch only (ADR-17), and `routes/enquiries.py` (ADR-18) are the exceptions: they call `database/supabase_client.py`'s `get_supabase_client()` instead, since `admins` reads/writes for login/register/forgot-password, Settings' password change, and all of Enquiries now go to Supabase (PostgreSQL), not SQLite. `routes/enquiries.py` also still calls `get_connection()` — not for its own reads, but to keep a SQLite mirror in sync for `routes/student.py`'s benefit (unmigrated, see TD-36).
