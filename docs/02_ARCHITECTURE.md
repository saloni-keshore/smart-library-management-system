...
3. Route opens a connection via `get_connection()`, or calls into a `database/*_queries.py` module (some blueprints use raw SQL inline instead, see [05_ROUTES_REFERENCE.md](05_ROUTES_REFERENCE.md) for which) — as of 2026-07-23, `routes/auth.py` (ADR-16), `routes/setting.py`'s `security_settings()` password branch only (ADR-17), `routes/enquiries.py` (ADR-18), `routes/student.py` (ADR-19), and `routes/membership.py` (ADR-20) are the exceptions: they call `database/supabase_client.py`'s `get_supabase_client()` instead, since `admins` reads/writes for login/register/forgot-password, Settings' password change, all of Enquiries, all of Students, and all of Memberships now go to Supabase (PostgreSQL), not SQLite. `routes/enquiries.py`, `routes/student.py`, and `routes/membership.py` all also still call `get_connection()` — not for their own reads, but to keep a SQLite mirror in sync for `routes/payment.py`/`routes/dashboard.py`/`routes/membership_distribution.py`/`routes/notification.py`/etc.'s benefit (all still unmigrated, and each still JOINs `students`/`enquiries`/`memberships` directly in SQL).
