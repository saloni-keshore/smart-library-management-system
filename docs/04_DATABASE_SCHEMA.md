...
**As of 2026-07-23 (ADR-18), `enquiries` is read/written in two places**, the same pattern as `admins` above: `routes/enquiries.py` (`index()`/`add()`/`edit()`/`delete()`/`view()`) reads/writes the copy in **Supabase** (schema defined by `database/supabase_migration.sql`) via `database/supabase_client.py`, and is the source of truth for every read. SQLite is kept as a write-synced **mirror** — `add()`/`edit()` write the identical row to both, `delete()` deletes from Supabase then best-effort deletes the SQLite mirror — purely because `routes/student.py`'s `admission()` reads the enquiry row and inserts `students` against a real SQLite FK to `enquiry_id` (`students` itself is now migrated too, ADR-19, but the SQLite mirror of both tables still exists for the other unmigrated modules described below). **As of 2026-07-23 (ADR-19), `admission()`'s own write, `enquiries.status = 'Admitted'`, goes to Supabase directly** (`.update({"status": "Admitted"}).eq("enquiry_id", ...)`), the same copy `routes/enquiries.py` reads — this closes TD-36 (now `Resolved`), which previously left that write SQLite-only while Supabase (and the Enquiries pages) never saw it. The SQLite mirror's own `status` column is no longer written by anything and is not read by anything either — every other column still mirror-syncs correctly (`full_name`/`mobile`/`purpose`/`preferred_shift`/`followup_date`/`remarks`), since `routes/enquiries.py` itself writes both copies for those.
...
**As of 2026-07-23 (ADR-20/ADR-21), `memberships` is read/written in two places**, the same pattern as `students`/`enquiries` above: `routes/membership.py` (`index()`/`create()`/`renew()`) and, as of ADR-21, `routes/payment.py`'s `collect()` too, all read/write the copy in **Supabase** via `database/supabase_client.py`, which is the source of truth for every read (`database/membership_queries.py`'s `get_active_membership()`, `routes/membership.py`'s only caller, reads Supabase too). SQLite is kept as a write-synced **mirror** — `create()`/`renew()` mirror every column of the new/updated row, and `collect()` mirrors its `paid_amount`/`pending_amount` update — purely because `routes/dashboard.py`, `routes/membership_distribution.py`, `routes/notification.py`, `routes/student.py`'s `view()`, and `database/cashbook_queries.py`'s `get_pending_fees()`/`database/bi_queries.py` (all unmigrated, out of scope for these slices) all still run raw SQL `JOIN memberships` queries directly against SQLite, and `database/membership_queries.py`'s other exports (`EFFECTIVE_STATUS_SQL`/`DAYS_LEFT_SQL`/`get_membership_counts`/`get_effective_status`, still SQL/SQLite-backed) are what those modules call. `create()`/`renew()` compute `membership_id` explicitly (`SELECT MAX(membership_id) FROM memberships` (SQLite) `+ 1`) rather than trusting Supabase's auto-assigned identity value, the same fix ADR-18/ADR-19 needed for `enquiry_id`/`student_id`. **TD-37 `Resolved` (2026-07-23, ADR-21):** `routes/payment.py`'s `collect()` now updates `paid_amount`/`pending_amount` in Supabase first (the source of truth), then mirrors the same update into SQLite — Supabase's copy no longer goes stale after a payment is collected against an existing membership.
...
**As of 2026-07-23 (ADR-22), `cashbook` is read/written in two places**, the same pattern as `enquiries`/`students`/`memberships` above: `routes/cashbook.py` (`index()`/`add_transaction()`/`edit_transaction()`) reads/writes the copy in **Supabase** (schema defined by `database/supabase_migration.sql`) via `database/supabase_client.py`, and is the source of truth for every read. SQLite is kept as a write-synced **mirror** — as of 2026-07-24 (ADR-26), purely for the `cashbook`↔`payments`↔`memberships`↔`students`↔`enquiries` FK chain itself (`audit_log.entry_id`'s FK no longer applies here: `database/audit_queries.py`'s `log_entry()` was deleted outright, the first mirror removed in Phase 10 — `cashbook_queries.py` writes the Supabase `audit_log` row directly on every cashbook write, with no SQLite `audit_log` row at all anymore). Manual entries (`insert_transaction()`) write Supabase first and roll back the SQLite mirror-write on failure, the same shape ADR-18/19/20 use; automatic entries (`insert_income_entry()`, triggered by Membership/Payment) keep the SQLite write as primary, unchanged, and best-effort mirror into Supabase afterward — that function's caller (`database/payment_queries.py`'s `record_payment()`, called from the unmigrated `routes/membership.py`/`routes/payment.py`) only catches `sqlite3.Error` and can't be given a new exception type without expanding scope. **As of 2026-07-24 (ADR-25), `payment_id` is sent to Supabase** for automatic entries too — `payments` itself migrated (best-effort mirror, ADR-25), so Supabase's `payment_id → payments` FK now has a real row to resolve against, closing TD-38's common case (best-effort staleness risk remains, TD-41, same class as TD-39).
