...
**As of 2026-07-23 (ADR-22), `cashbook` is read/written in two places**, the same pattern as `enquiries`/`students`/`memberships` above: `routes/cashbook.py` (`index()`/`add_transaction()`/`edit_transaction()`) reads/writes the copy in **Supabase** (schema defined by `database/supabase_migration.sql`) via `database/supabase_client.py`, and is the source of truth for every read. SQLite is kept as a write-synced **mirror** — purely because `audit_log.entry_id` is a real SQLite FK to `cashbook.entry_id`, and `database/audit_queries.py`'s `log_entry()` still inserts a SQLite `audit_log` row on every cashbook write. Manual entries (`insert_transaction()`) write Supabase first and roll back the SQLite mirror-write on failure, the same shape ADR-18/19/20 use; automatic entries (`insert_income_entry()`, triggered by Membership/Payment) keep the SQLite write as primary, unchanged, and best-effort mirror into Supabase afterward — that function's caller (`database/payment_queries.py`'s `record_payment()`, called from the unmigrated `routes/membership.py`/`routes/payment.py`) only catches `sqlite3.Error` and can't be given a new exception type without expanding scope. **`payment_id` is never sent to Supabase** for automatic entries — Supabase enforces the same `payment_id → payments` FK shown above, and `payments` is still SQLite-only, so a real `payment_id` there always fails (`23503`, verified live); the SQLite mirror keeps the real value (TD-38 in [11_FUTURE_WORK.md](11_FUTURE_WORK.md)). The best-effort mirror-write can also leave a narrow, bounded staleness window on Supabase after a transient failure (TD-39).
...
**As of 2026-07-23 (ADR-22), reads move to Supabase, but the SQLite write is entirely unchanged:** `database/audit_queries.py`'s `get_recent_audit_log()` now reads the copy in **Supabase** via `database/supabase_client.py` — the source of truth for `routes/cashbook.py`'s "Audit Trail" activity log. `log_entry(cursor, ...)` itself still only writes SQLite, exactly as before this migration — `database/cashbook_queries.py` writes the matching Supabase row itself, next to its own Supabase `cashbook` write (strict for manual entries, best-effort for automatic ones — see `cashbook`'s note above). The SQLite `audit_log` mirror this leaves behind has **zero application-level readers** — it's kept solely because `admin_id`/`entry_id` are real SQLite FKs (to `admins`/`cashbook`) that still fire on every write; see [MIRROR_TRACKER.md](MIRROR_TRACKER.md)'s `audit_log` section for its exact removal conditions.
