...
- **Security Settings** *(new 2026-07-21)*: `routes/setting.py`'s `security_settings()` → `database/security_settings_queries.py` (session preferences) + raw SQL on `admins` (password change) → `templates/settings/security_settings.html` → `static/js/settings.js` (new/confirm password match check). Password change is fully functional (verified end-to-end: wrong password rejected, correct change works, can log in with the new password). Session timeout / remember-me / login-notification preferences are persisted via its own `security_settings` table (ADR-9) but **not enforced** anywhere yet (TD-26).
