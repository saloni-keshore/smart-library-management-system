...
graph TD
    ...
    Root --> PandaSpec["PANDA_SPEC.md (corrected Panda spec, replaces an external PDF)"]
    Root --> Database["database/"]
    Root --> Routes["routes/"]
    Root --> Panda["panda/ (self-contained blueprint package, ADR-43/47/48/49/50/51)"]
    ...
    Database --> DBQueries["*_queries.py + cashbook_categories.py (11 modules, incl. panda_queries.py)"]
    Database --> DBFile[("library.db")]

    Routes --> RouteFiles["14 blueprint modules, see FILE_REFERENCE.md"]
...
graph TD
    ...
    subgraph DataAccess ["Data Access Layer (database/)"]
        ...
        BiQ["bi_queries.py"]
        AICenterQ["ai_center_queries.py"]
        AICenterSettingsQ["ai_center_settings_queries.py"]
        PandaQ["panda_queries.py"]
    ...
    BI --> BiQ
    AICenter --> AICenterQ
    AICenter --> AICenterSettingsQ
    AICenterQ --> MembershipQ
    AICenterQ --> AICenterSettingsQ
    AICenterQ --> SupabaseClient
    AICenterSettingsQ --> SupabaseClient
    Panda --> BiQ
    Panda --> Notification
    Panda --> AICenterQ
    Panda --> MembershipQ
    Panda --> PandaQ
    PandaQ --> SupabaseClient
...
Also as of 2026-07-27 (Panda AI Assistant, Phase 1, ADR-43, unrelated to the SQLite→Supabase migration above): a 15th blueprint, `Panda` (`panda/routes.py`), was added — the first blueprint in this diagram that is itself a small subgraph of modules (`panda/services.py`/`prompts.py`/`notifications.py`/`insights.py`/`forecasting.py`, collapsed into the single `Panda` node above for readability) rather than one file talking directly to `DataAccess`. `Panda --> BiQ` and `Panda --> Notification` are read-only reuse (via `panda/insights.py` wrapping `get_revenue_growth`/`get_occupancy_summary`/`get_notification_summary` — no new query logic), and `Panda --> PandaQ --> SupabaseClient` is Panda's own chat-history persistence (`panda_conversations`/`panda_messages`, new tables, same "not yet applicable by this app itself" manual-creation status `ai_center_settings` has — see TD-55). `panda/forecasting.py` was unreachable stub code at this point, called from nowhere - see 2026-08-19 (Part 5) below for when that changed.

Also as of 2026-08-18 (Panda Part 1 "Ask", ADR-47): `panda/intents.py` joins the collapsed `Panda` node's module list — it sits between `panda/services.py` and `panda/prompts.py`/`panda/insights.py` (`services.py`'s `send_message()` now calls `intents.generate_reply()` instead of `prompts.generate_placeholder_reply()` directly; `intents.py` itself calls `panda/insights.py` for the three answerable intents' data and falls through to `prompts.generate_placeholder_reply()` for anything it doesn't recognize). No new edge to `BiQ`/`Notification`/`PandaQ`/`SupabaseClient` — `intents.py` reuses the exact same `panda/insights.py` reads Panda's insights cards already used, not a new query path.

Also as of 2026-08-18 (Panda Part 2 "Analyze", ADR-48): `Panda` gains two new edges, `Panda --> AICenterQ` and `Panda --> MembershipQ` — `panda/insights.py`'s new `get_top_risk_students()` loops `AICenterQ`'s existing single-student `compute_student_risk()` over the roster `MembershipQ`'s `get_admin_students()` returns, to answer "which students are likely to leave?" for real instead of redirecting. `panda/insights.py` also gained two pure wrappers around `BiQ` functions already in scope (`get_monthly_new_memberships`, `get_purpose_breakdown`) — no new edge for those, since `Panda --> BiQ` already exists.

Also as of 2026-08-19 (Panda Part 3 "Explain", ADR-49): `panda/insights.py`'s new `get_revenue_explanation()` adds one new edge, `panda_insights_py --> cashbook_queries_py` (`database.cashbook_queries.get_pending_fees()`) — its other two signals (admissions momentum, unrenewed expirations) reuse `get_admissions_trend()`/`get_expiring_memberships()`, already-existing functions with no new edges of their own. `panda/intents.py` gains no new edges — `_reply_explain_revenue()` calls `panda/insights.py` the same way every other `_reply_*` handler does.

Also as of 2026-08-19 (Panda Part 4 "Recommend", ADR-50): `panda/insights.py`'s new `get_recommended_actions()` calls `database/bi_queries.py`'s `get_action_items()` — no new edge, since `Panda --> BiQ` (collapsed) / `panda_insights_py --> bi_queries_py` (literal) already exists from Part 1 onward. `panda/intents.py` gains no new edges either — `_reply_recommend()` calls `panda/insights.py` the same way every other `_reply_*` handler does.

Also as of 2026-08-19 (Panda Part 5 "Forecast", ADR-51): `panda/forecasting.py` goes from unreachable stub to real, real edges: `panda_forecasting_py --> bi_queries_py` (`last_n_months`, `get_monthly_occupancy_trend`, `get_upcoming_expiries`), `panda_forecasting_py --> cashbook_queries_py` (`get_monthly_income`), `panda_forecasting_py --> membership_queries_py` (`get_memberships_for_admin`). `panda/insights.py` gains one new edge into it, `panda_insights_py --> panda_forecasting_py` (three thin wrappers - `get_revenue_forecast`/`get_seat_demand_forecast`/`get_churn_forecast`), the collapsed `Panda` node's first internal edge shown explicitly rather than folded into the node. `panda/intents.py` gains no new edges — `_reply_forecast()` calls `panda/insights.py` the same way every other `_reply_*` handler does.
...
erDiagram
    ...
    PANDA_CONVERSATIONS {
        ...
        int admin_id FK
        text title "nullable, auto-set from first user message"
    }
    PANDA_MESSAGES {
        int message_id PK
...
`settings` (legacy) and `transactions` (defined twice, see [04_DATABASE_SCHEMA.md](04_DATABASE_SCHEMA.md)) are omitted here since neither is used by any route today — see [11_FUTURE_WORK.md](11_FUTURE_WORK.md) TD-2/TD-4. `PANDA_CONVERSATIONS`/`PANDA_MESSAGES` (added 2026-07-27, ADR-43) are brand-new tables not yet applicable by this app itself on any given Supabase project — see TD-55 — same "documented in `supabase_migration.sql`, needs a manual `CREATE TABLE`" status as `ai_center_settings` (TD-53, `Resolved`), not omitted like the genuinely-unused legacy tables above.

## 5. Module dependency graph (literal Python imports, verified by grep on 2026-07-20, updated 2026-07-21 for `database/membership_queries.py`, updated 2026-07-22 for `database/payment_queries.py`, updated 2026-07-23 for `database/supabase_client.py` and its `routes/membership.py` (ADR-20), `routes/payment.py` (ADR-21), `database/cashbook_queries.py`/`database/audit_queries.py` (ADR-22), and the analytics layer's cutover to `database/membership_queries.py`'s Supabase-backed helpers (ADR-23), updated 2026-07-24 for all six Settings query modules' cutover to `database/supabase_client.py` (ADR-24) and `database/payment_queries.py`'s cutover, closing every mirror's read-side (ADR-25), updated 2026-07-27 to add `routes/ai_center.py`/`database/ai_center_queries.py` (AI Center, Phase 1, ADR-39) and `database/ai_center_settings_queries.py` (AI Center Settings, ADR-40), updated 2026-07-27 to add the `panda/` package and `database/panda_queries.py` (Panda AI Assistant, Phase 1, ADR-43), updated 2026-08-18 to add `panda/intents.py` (Panda Part 1 "Ask", ADR-47), updated 2026-08-18 to add `panda_insights_py --> ai_center_queries_py`/`panda_insights_py --> membership_queries_py` (Panda Part 2 "Analyze", ADR-48), updated 2026-08-19 to add `panda_insights_py --> cashbook_queries_py` (Panda Part 3 "Explain", ADR-49), updated 2026-08-19 for Panda Part 4 "Recommend" (ADR-50), no new edge — `panda_insights_py --> bi_queries_py` already existed, updated 2026-08-19 to add `panda_insights_py --> panda_forecasting_py`, `panda_forecasting_py --> bi_queries_py`, `panda_forecasting_py --> cashbook_queries_py`, `panda_forecasting_py --> membership_queries_py` (Panda Part 5 "Forecast", ADR-51, completing the Ask→Analyze→Explain→Recommend→Forecast progression), updated 2026-08-20 for Panda's retention/cash topic expansion (ADR-52, not a sixth part) — no new edge, `panda_insights_py --> bi_queries_py`/`panda_insights_py --> membership_queries_py` already existed)
...
graph LR
    ...
    subgraph Routes["routes/"]
        ...
        ai_center_py["ai_center.py"]
        notification_py["notification.py"]
        setting_py["setting.py"]
        report_py["report.py"]
    end

    subgraph Panda["panda/"]
        panda_routes_py["routes.py"]
        panda_services_py["services.py"]
        panda_intents_py["intents.py"]
        panda_prompts_py["prompts.py"]
        panda_notifications_py["notifications.py"]
        panda_insights_py["insights.py"]
        panda_forecasting_py["forecasting.py (real as of Part 5, ADR-51)"]
    end
    ...
    subgraph DB["database/"]
        ...
        ai_center_queries_py["ai_center_queries.py"]
        ai_center_settings_queries_py["ai_center_settings_queries.py"]
        panda_queries_py["panda_queries.py"]
    ...
    ai_center_py --> ai_center_queries_py
    ai_center_py --> ai_center_settings_queries_py
    ai_center_queries_py --> membership_queries_py
    ai_center_queries_py --> ai_center_settings_queries_py
    ai_center_queries_py --> supabase_client_py
    ai_center_settings_queries_py --> supabase_client_py
    panda_routes_py --> panda_services_py
    panda_services_py --> panda_notifications_py
    panda_services_py --> panda_intents_py
    panda_services_py --> panda_prompts_py
    panda_services_py --> panda_queries_py
    panda_intents_py --> panda_insights_py
    panda_intents_py --> panda_prompts_py
    panda_notifications_py --> panda_insights_py
    panda_insights_py --> bi_queries_py
    panda_insights_py --> notification_py
    panda_insights_py --> ai_center_queries_py
    panda_insights_py --> membership_queries_py
    panda_insights_py --> cashbook_queries_py
    panda_insights_py --> panda_forecasting_py
    panda_forecasting_py --> bi_queries_py
    panda_forecasting_py --> cashbook_queries_py
    panda_forecasting_py --> membership_queries_py
    panda_queries_py --> supabase_client_py
