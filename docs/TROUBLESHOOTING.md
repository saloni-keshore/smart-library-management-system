...
**Cause (fixed 2026-09-09):** the floating Panda widget button (`components/panda_widget.html`, `panda.css`'s `.panda-toggle-btn` — `position: fixed; right:24px; bottom:24px; z-index:1055`, loaded globally on every logged-in page) sits in the same on-screen corner a page's trailing bottom-right button lands in once scrolled into view. On a short form — most visibly `templates/memberships/create.html`'s "Save & Continue" when no shift slots/extra charges are configured — that button could render directly underneath the widget's 60px-diameter hit area, since `.content-wrapper` only reserved 24px of bottom padding. A click there was silently captured by the widget instead of the form: no page navigation, no validation tooltip, no error message — just an unresponsive-looking button. See the 2026-09-09 CHANGELOG entry and **TD-97** in [11_FUTURE_WORK.md](11_FUTURE_WORK.md).
**If you still see this:** confirm `.content-wrapper`'s bottom padding (`static/css/style.css`) is still comfortably larger than the widget's footprint (60px button + 24px offset) — it can regress if either value changes without the other being revisited (TD-97). Quick way to confirm this specific cause: check whether the browser's address bar changes at all when clicking, and whether any native "please fill out this field" tooltip appears — if neither happens, the click never reached the form at all, which points at something (like this widget) intercepting it rather than a server-side validation/flash issue.
