...
## `routes/auth.py`/`routes/setting.py`/`routes/enquiries.py` returns a 500 / unhandled `postgrest.exceptions.APIError` on login, register, forgot-password, Security Settings password change, or Enquiries add/edit/delete/view

**Cause:** `database/supabase_client.py`'s `get_supabase_client()` raises `RuntimeError` (not `APIError`) if `SUPABASE_URL`/`SUPABASE_SECRET_KEY` aren't set — check `.env` first. If those are set, `routes/auth.py` (ADR-16), `routes/setting.py`'s `security_settings()` password branch (ADR-17), and `routes/enquiries.py` (ADR-18, all 2026-07-23) each wrap every Supabase call that carries user-controlled input in `try/except APIError` (added specifically because Supabase's Cloudflare-fronted edge network was observed to 403-block PostgREST `.eq()`/`.select()` GET requests whose query string looks like a SQL-injection payload, e.g. contains `DROP TABLE` — a failure mode SQLite's parameterized queries never had). A 500 here means either a genuinely new, unhandled Supabase error shape, or a code path that doesn't yet have the `try/except` — compare against ADR-16/ADR-17/ADR-18 in [DECISIONS.md](DECISIONS.md) for which calls are covered.
**Where it surfaces:** `POST /`, `POST /register`, `POST /forgot-password`, `POST /settings/security` (`form_type=password`), `GET /enquiries/`, `POST /enquiries/add`, `GET/POST /enquiries/edit/<id>`, `GET/POST /enquiries/delete/<id>`, `GET /enquiries/view/<id>`.

## Enquiries list/detail still shows "Interested" (or the pre-admission status) after a student was just admitted

**Cause — this is a live, known, temporary issue (TD-36 in [11_FUTURE_WORK.md](11_FUTURE_WORK.md), Open):** as of 2026-07-23 (ADR-18), `routes/enquiries.py`'s `index()`/`edit()`/`view()` read `enquiries.status` from **Supabase**, while `routes/student.py`'s `admission()` (unmigrated, out of that session's scope) still runs `UPDATE enquiries SET status='Admitted' ...` against the **SQLite** mirror only. The two copies agree on `full_name`/`mobile`/`purpose`/`preferred_shift`/`followup_date`/`remarks` (kept in sync by `routes/enquiries.py`'s own `add()`/`edit()`), but not on `status`, which only `routes/student.py` ever writes.
**Fix:** none yet from the Enquiries side — the `student_id` badge/link (read separately from SQLite) does correctly reflect the admission, so "View Student" is reachable even though the status badge and action link are stale. The real fix is migrating `routes/student.py`'s `students`/`enquiries` reads to Supabase (the recommended next slice per ADR-18), so `admission()` writes `status` to the same place the Enquiries pages read it from.
