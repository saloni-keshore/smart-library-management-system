...
## `routes/auth.py`/`routes/setting.py`/`routes/enquiries.py`/`routes/student.py`/`routes/membership.py` returns a 500 / unhandled `postgrest.exceptions.APIError` on login, register, forgot-password, Security Settings password change, Enquiries add/edit/delete/view, Students index/admission/view/edit, or Memberships index/create/renew

**Cause:** `database/supabase_client.py`'s `get_supabase_client()` raises `RuntimeError` (not `APIError`) if `SUPABASE_URL`/`SUPABASE_SECRET_KEY` aren't set — check `.env` first. If those are set, `routes/auth.py` (ADR-16), `routes/setting.py`'s `security_settings()` password branch (ADR-17), `routes/enquiries.py` (ADR-18), `routes/student.py` (ADR-19), and `routes/membership.py`/`database/membership_queries.py`'s `get_active_membership()` (ADR-20, all 2026-07-23) each wrap every Supabase call that carries user-controlled input in `try/except APIError` (added specifically because Supabase's Cloudflare-fronted edge network was observed to 403-block PostgREST `.eq()`/`.select()` GET requests whose query string looks like a SQL-injection payload, e.g. contains `DROP TABLE` — a failure mode SQLite's parameterized queries never had). A 500 here means either a genuinely new, unhandled Supabase error shape, or a code path that doesn't yet have the `try/except` — compare against ADR-16/ADR-17/ADR-18/ADR-19/ADR-20 in [DECISIONS.md](DECISIONS.md) for which calls are covered.
**Where it surfaces:** `POST /`, `POST /register`, `POST /forgot-password`, `POST /settings/security` (`form_type=password`), `GET /enquiries/`, `POST /enquiries/add`, `GET/POST /enquiries/edit/<id>`, `GET/POST /enquiries/delete/<id>`, `GET /enquiries/view/<id>`, `GET /students/`, `GET/POST /students/admission/<id>`, `GET /students/view/<id>`, `GET/POST /students/edit/<id>`, `GET /memberships/`, `GET/POST /memberships/create/<id>`, `GET/POST /memberships/renew/<id>`.

## A membership's `paid_amount`/`pending_amount` on the Memberships page (Supabase) looks stale right after collecting a payment on the Payments page

**Cause — a new, real split-brain (TD-37):** as of 2026-07-23 (ADR-20), `routes/membership.py`'s `index()` reads `memberships` from **Supabase**, but `routes/payment.py`'s `collect()` (unmigrated, out of that session's scope) still writes the pending-balance update (`paid_amount`/`pending_amount`) to the **SQLite** mirror only. A payment collected via `/payments/collect/<id>` updates the SQLite copy correctly (and everything that reads SQLite — `routes/dashboard.py`, `routes/membership_distribution.py`, `routes/notification.py`, the Student detail page — sees it immediately), but Supabase's copy of that same row keeps the pre-payment `paid_amount`/`pending_amount` until the next time `routes/membership.py`'s `create()`/`renew()` writes that row (which doesn't happen from a payment collection).
**Fix:** none yet — the real fix is migrating `routes/payment.py` to Supabase (the natural next slice per ADR-19's original Membership+Payment pairing suggestion, deferred this session per explicit scope), so `collect()` writes the same copy `routes/membership.py`'s `index()` reads.
