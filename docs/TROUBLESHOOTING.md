...
## A bottom-right submit button (e.g. "Save & Continue") appears to do nothing when clicked

**Cause (fixed 2026-09-09):** the floating Panda widget button (`components/panda_widget.html`, `panda.css`'s `.panda-toggle-btn` — `position: fixed; right:24px; bottom:24px; z-index:1055`, loaded globally on every logged-in page) sits in the same on-screen corner a page's trailing bottom-right button lands in. The real overlap was **horizontal**, not just vertical: `.content-wrapper` had no right padding at all (`width: calc(100% - 248px)` runs flush to the viewport edge), so a right-aligned button (the `d-flex justify-content-between` "Back" / "Save & Continue" pattern) rendered within the widget's ~84px-wide footprint regardless of scroll position — most visibly `templates/memberships/create.html`'s "Save & Continue", confirmed live via a screenshot showing the widget overlapping the button's arrow icon. A click on the overlapped pixels was silently captured by the widget instead of the form: no page navigation, no validation tooltip, no error message — just an unresponsive-looking button. An initial fix only widened the *bottom* padding, which didn't resolve it (the overlap wasn't about scroll depth); the real fix added right-side padding too. See both 2026-09-09 CHANGELOG entries and **TD-97** in [11_FUTURE_WORK.md](11_FUTURE_WORK.md).
**If you still see this:** as of 2026-09-11, `.content-inner`'s padding (`static/css/style.css`) reads `--panda-clearance-right`/`--panda-clearance-bottom` directly from `static/css/panda.css` instead of hardcoding its own numbers, so the two can no longer silently drift apart (TD-97, now Resolved) — if the widget's size or offset ever changes, only `panda.css`'s two custom properties need updating. Quick way to confirm this specific cause: check whether the browser's address bar changes at all when clicking, and whether any native "please fill out this field" tooltip appears — if neither happens, the click never reached the form at all, which points at something (like this widget) intercepting it rather than a server-side validation/flash issue.
**Where it surfaces:** any page whose primary action button is bottom-right and ends up as the last thing on a short page — reported first on Create Membership.

## "Save & Continue" / "Collect Payment" appears to hang or do nothing, even though the button itself is fine

**Cause (fixed 2026-09-09 for this call path):** same class of bug as TD-70/ADR-54 (2026-08-24), which fixed `database/panda_queries.py` but never touched the Membership/Payment routes. `routes/membership.py`'s `create()`/`renew()` and `routes/payment.py`'s `collect()` caught `postgrest.exceptions.APIError` only - that covers an HTTP error response from Supabase, but not `httpx.TransportError` (a dropped connection, a timeout). A transient network blip during any of these routes' many sequential Supabase calls (confirmed live: creating one membership makes roughly 18-20 separate round trips) crashed the whole request unhandled instead of showing a clean "please try again" flash - the admin just saw an unresponsive page with no error, no matter how long they waited, since the crash happened server-side partway through, not client-side. See the 2026-09-09 CHANGELOG entry and **TD-99** in [11_FUTURE_WORK.md](11_FUTURE_WORK.md).
**If you still see this:** the underlying slowness (5-10+ seconds per request, from the sheer number of sequential Supabase calls) is not fixed, only the unhandled-crash symptom is - a request can still be *slow*, it just shouldn't crash silently on a connection blip anymore. If it still hangs with no flash message at all afterward, check the deployment's logs for a traceback naming an exception type this fix doesn't cover (TD-99's fix only widened `except APIError` to also catch `httpx.TransportError` in this one call path - other exception types, or Supabase call sites elsewhere in the codebase, are still `APIError`-only).
**Where it surfaces:** `routes/membership.py` (`create`, `renew`), `routes/payment.py` (`collect`) - reported first on Create Membership.

## "Save & Continue" on Create/Renew Membership does nothing — no receipt, no page change, and *no error banner at all*

**Cause (TD-100; root removed 2026-09-10, ADR-72):** the shift-slots feature (ADR-65) added a Night hours input — `<input type="number" id="night_hours" min="1" value="0">` — inside a `d-none` wrapper that inline JS only un-hid for a night-hourly slot. Hidden but not `disabled`, it still failed HTML5 validation (`value="0"` violates `min="1"`), so the whole `<form>` was `:invalid` and the browser **silently refused to submit** — a `display:none` field can't be focused for a tooltip, no POST leaves the browser, so no server `flash()` ever runs (hence no red banner). It was first patched by toggling `#night_hours`'s `disabled`; **as of ADR-72 the field and its entire night-hourly billing mode are deleted** — a night shift is just a shift priced at its monthly fee. Separate earlier causes of the same symptom: the Panda-widget overlap (TD-97) and the `httpx.TransportError` crash (TD-99).
**Two layers of protection now:** (1) the offending field is gone; (2) `templates/layouts/base.html` has an app-wide capture-phase `invalid` listener — any control that fails validation while invisible (`offsetParent === null`) is disabled and the form re-submitted once, so a *new* hidden constrained field can't reintroduce this class.
**If a Continue/Submit button still looks dead:** open DevTools → Console. A `"not focusable"` line naming a field = a hidden invalid field the guard didn't catch (report it). A red JS error = the page's inline script threw before wiring up — fix that. Nothing at all in the console + no Network POST = something is intercepting the click (see the TD-97 Panda-widget entry) or the fix simply isn't deployed yet (the working-tree change must be committed and pushed for Vercel to pick it up).
**Where it surfaces:** `templates/memberships/create.html`, `templates/memberships/renew.html` — the Membership & Payment step.

## On Create Membership, selecting Day Pass / Custom / Custom hours — the "Plan Fee (₹)" box won't accept typing

**Cause (fixed 2026-09-12, TD-107):** the "Plan Fee (₹)" box in the Fee & Charges card is `readonly` by design — it's meant to be a computed display for the five fixed plans and real shift slots. Before this fix, selecting Day Pass / Custom / a "Custom hours" shift blanked that box (so it looked like an empty field waiting for input, and would even show a focus ring when clicked) but never removed `readonly`, so typing into it silently did nothing. The field that actually needed the typed amount, `total_fee`, was a **different** input entirely — inside the "Total Payable" field of the sidebar's Payment Summary card — with no visible cue pointing there.
**If you still see this:** confirm you're on `templates/memberships/create.html` after 2026-09-12 — the fix makes the Plan Fee box itself become a real, enabled number input (with a "Type the fee to charge." hint underneath) the moment a manual-fee plan/shift is selected, and it's now the one field posted as `total_fee`. `renew.html` never had this bug (it has no separate Plan Fee box).
**Where it surfaces:** `templates/memberships/create.html` — the "Membership Plan" = Day Pass/Custom, or "Shift" = Custom hours, cases only; fixed plans and real shift slots were never affected.

## The "Casual" badge / Regular/Casual filter on the Students list does nothing, or the Day Pass fee always shows 0

**Cause (expected until the migration runs — TD-102):** the Walk-in / Day Pass feature (ADR-71) needs three columns the app cannot create itself (ADR-14): `students.student_type`, `membership_settings.day_pass_fee`, `membership_settings.day_pass_days`. Until an operator runs the `ALTER`s (in `database/supabase_migration.sql`'s comment blocks) in the Supabase SQL Editor: every student reads as `Regular` (so the Casual filter's view is always empty and no "Casual" badge appears), and the Day Pass plan pre-fills fee `0` / `1` day. **Quick Admit and selling a Day Pass still work** — `routes/student.py`'s `_insert_student_row()` and `save_membership_settings()` strip the missing column and retry.
**Fix:** run `ALTER TABLE students ADD COLUMN IF NOT EXISTS student_type TEXT DEFAULT 'Regular';` and `ALTER TABLE membership_settings ADD COLUMN IF NOT EXISTS day_pass_fee DOUBLE PRECISION NOT NULL DEFAULT 0, ADD COLUMN IF NOT EXISTS day_pass_days INTEGER NOT NULL DEFAULT 1;`, then set the fee in Settings → Membership Settings → "Walk-in / Day Pass".
**Not this:** a walk-in you registered earlier still shows as `Regular` even after the migration — `student_type` only defaults on *new* rows; edit the student or re-run Quick Admit is not needed, but the column won't retro-fill. A one-off `UPDATE students SET student_type = 'Casual' WHERE ...` is the only way to backfill.

## A walk-in / Day Pass payment doesn't show in Dashboard "Total Revenue" or the collection-rate

**Cause (by design — ADR-71 / ADR-11):** Day Pass income is booked to its own `"Day Pass"` **Cashbook** category, not the `payments` table. Dashboard's "Total Revenue" / "Today's Collection" and the Business Health collection-rate are deliberately `payments`-only ("money collected from *members*"), so a Day Pass shows in **Cashbook** total income and the BI revenue-source breakdown, not those member-fee cards. This is the same split that keeps donations / fines / book sales out of "fee revenue".
**If a Day Pass is missing from the Cashbook too:** the `"Day Pass"` ledger write is best-effort (`insert_income_entry`, no `cashbook_synced` recovery flag), so a transient Supabase failure during the sale can leave the paid Day Pass membership with no ledger row — check the deployment logs, and add the income manually as "Other Income" if needed (TD-102).
...
## Top nav badge counts (enquiries/students/memberships/payments) show blank or zero

**Cause:** `templates/layouts/navbar.html` (single top nav as of 2026-09-11, ADR-73 — this lived in the now-deleted `sidebar.html` before) references `enquiries_new_count`, `students_new_today_count`, `memberships_expiring_soon_count`, `payments_pending_count`, but no route or context processor currently supplies them (Known Technical Debt item TD-15) — this isn't a bug you introduced, it's a pre-existing gap.
...
## Changes made locally don't show up on the Vercel (or Render) site, or vice versa

**Cause:** local dev and the deployment are pointed at **different** Supabase projects. As of ADR-69 they're meant to share one — local dev is just another host of the deployment's project.
**Fix:** compare the `Connected Supabase project: ...<last-24-chars>` startup line in the local console against the same line in `vercel logs` / the Render log. If they differ, set `SUPABASE_URL`/`SUPABASE_SECRET_KEY` in local `.env` and in the Vercel/Render dashboards to the *same* project (schema applied — `python scripts/verify_schema.py`). See [DEPLOYMENT.md](DEPLOYMENT.md)'s Vercel section. If you'd rather local dev not touch production, use a dedicated shared dev/staging project (TD-98).

## `pytest` created QA admins / enquiries / memberships in my real Supabase project

**Cause:** the suite ran against whatever `.env` points at because there was no `.env.test`. `tests/conftest.py` has no mocking or teardown — every run registers a fresh admin and creates real rows (ADR-69).
**Fix:** create `.env.test` (git-ignored — copy `.env.test.example`) with a **dedicated throwaway** project's `SUPABASE_URL`/`SUPABASE_SECRET_KEY`; `conftest.py` loads it with `override=True` before the app imports, so the suite binds there. To clean up rows already written to the shared project, filter on the `qa_*` username prefix / `QA Tester` names the fixtures use. Use `python scripts/verify_schema.py` (read-only) instead of `pytest` when you just need to check a real project's schema.

## A query returns empty, or fewer rows than expected, after the ADR-75 multi-tenant conversion

**Cause:** as of 2026-09-16 (ADR-75), every tenant-owned table is protected by Postgres Row-Level Security, and per-request app traffic authenticates as the `authenticated`/`anon` role via a signed JWT (`database/supabase_client.py`'s `build_tenant_client()`/`build_anon_client()`), never the service-role key. **RLS denial looks exactly like "no rows matched" — it never raises an error or a distinguishable status code.** The most common causes, in order of likelihood: (1) `SUPABASE_JWT_SECRET` or `SUPABASE_ANON_KEY` is unset/wrong for this environment — a JWT signed with the wrong secret, or a client built with the wrong anon key, fails Supabase's own auth check before RLS is even evaluated, which also reads as an empty result, not an error; (2) `database/supabase_rls_migration.sql` hasn't been applied to this project yet, or was applied only partially (its own header comment documents a specific apply order — schema/backfill, then RLS+RPCs); (3) a genuinely new table was given an `admin_id` column but never added to that file's RLS-enable loop — Postgres's own RLS default is deny-all once RLS is turned on for a table with zero policies, so a table with RLS enabled and no policy denies everyone, indistinguishable from "the schema truly has no matching rows" without checking the SQL Editor directly; (4) **as of 2026-09-18 (ADR-77):** the admin's own `admins.status` is `'archived'` — `current_admin_id()` now returns `NULL` for that admin regardless of how valid their JWT otherwise is, which also reads as "no rows matched" everywhere, not just at login (see the archived-account entry below).
**Fix:** confirm `SUPABASE_JWT_SECRET`/`SUPABASE_ANON_KEY`/`SUPABASE_URL` are all set and correct for the project this code is actually running against (`app.py`'s startup log line, `Connected Supabase project: ...`, confirms the project — it does not confirm the JWT secret matches). Run `python scripts/verify_tenant_isolation.py` against the same project — besides proving isolation, an "INCONCLUSIVE — tenant A's own row wasn't returned either" result for a specific table is direct evidence of exactly this class of misconfiguration (wrong secret/key, or RLS/policy not applied), not a leak. If a specific table is the one degrading, check the Supabase SQL Editor (`SELECT * FROM pg_policies WHERE tablename = '...'`) for a `tenant_isolation` policy — if it's genuinely missing, re-run the relevant section of `database/supabase_rls_migration.sql`.
**Where it surfaces:** any route or `database/*_queries.py` module, but most confusing on a page that used to show data and now silently shows none — check this before assuming the underlying data was deleted or the query logic changed.

## Can't log in after archiving my account (or: "This account has been archived" on login)

**Cause:** this is expected behavior, not a bug — Settings → Data & Backup's Danger Zone → **Archive My Library** (ADR-76) sets `admins.status = 'archived'` for that account. `routes/auth.py`'s `login()` calls `rpc_login_lookup()`, which returns `status` alongside the usual fields; when it's `"archived"`, `login()` shows a distinct flash ("This account has been archived and can no longer be accessed...") and never attempts a password check at all — the account's data is untouched, but no session can be created for it.
**Fix:** there is currently **no self-service or admin-facing route that reactivates an archived account** — nothing clears `admins.status`/`archived_at` back to `'active'` (tracked as PF-9 in [11_FUTURE_WORK.md](11_FUTURE_WORK.md)). The only way back in today is a direct `UPDATE admins SET status = 'active', archived_at = NULL WHERE admin_id = ...` run by hand in the Supabase SQL Editor. If the archive was intentional and permanent, the correct next step is **Permanently Delete** instead (same Danger Zone), not attempting to log back in.
**Where it surfaces:** `routes/auth.py`'s `login()`; the confirmation flow itself is `routes/setting.py`'s `backup_archive()`/`templates/settings/data_backup.html`. **As of 2026-09-18 (ADR-77):** archiving now also revokes access immediately for a session/browser tab that was already logged in when the archive happened (previously that session kept working until its short-lived tenant JWT/session expired) — `current_admin_id()` returns `NULL` for an archived admin, so every RLS-protected query and both ADR-76 data-lifecycle RPCs go empty/fail for that session right away, not just new login attempts. This can look like "my dashboard suddenly shows nothing" rather than an explicit archived message, since the app-layer `login_required` check only looks at `session["admin_id"]` being present, not the admin's current DB status — the session cookie itself isn't cleared until that admin's next `/logout` or a fresh login attempt.
