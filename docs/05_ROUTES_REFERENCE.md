...
|---|---|---|---|---|---|
| GET/POST | `/` | `login` | `auth/login.html` | No | Login by username or mobile + password. Signup is always available now (`show_signup=True` unconditionally, ADR-75) — self-registration is no longer gated on the database being empty. An account with `admins.status = 'archived'` (ADR-76) is refused with a distinct message instead of the generic invalid-credentials one |
| GET | `/logout` | `logout` | redirect | No | Clears session, redirects to `/` |
| GET/POST | `/register` | `register` | `auth/register.html` | No | Creates a new admin account. Always open (ADR-75) — the old "only during initial setup"/`admin_count` gate is gone; any number of libraries can self-register into the shared database |
...
Data access: as of 2026-09-16 (ADR-75), this route no longer queries `admins` directly at all — the anon-role, pre-login Supabase client (`database.supabase_client.build_anon_client()`, wired in automatically by `app.py`'s `attach_tenant_supabase_client` before_request hook) has no table-level RLS grant on `admins`, so `login`/`register`/`forgot_password` instead call four `SECURITY DEFINER` Postgres RPCs via `supabase.rpc(...)`: `rpc_login_lookup`, `rpc_register_admin`, `rpc_forgot_password_lookup`, `rpc_reset_password` (all defined in `database/supabase_rls_migration.sql`). This replaces the previous direct `.table("admins")` `.eq()`-filtered queries (ADR-16) — see ADR-75/ADR-2 in [DECISIONS.md](DECISIONS.md) for why a tenant-scoped RLS policy can't express "look up by username/mobile before any tenant identity exists." Password validation (`validate_password`): min 8 chars, ≥1 letter, ≥1 digit. `register`'s username/mobile uniqueness check and insert now happen atomically inside `rpc_register_admin()` itself (closing a pre-existing TOCTOU race as a side effect), and it still hardcodes `role="Admin"` on insert (note: schema default is lowercase `'admin'` — inconsistent casing in practice, TD-13).
...
| GET/POST | `/enquiries/add` | `add` | `enquiries/add.html` | Yes | Create enquiry — **if the phone number is already on file, creates nothing and redirects to that person's `enquiry.view` / `student.view`** (ADR-58) |
| GET/POST | `/enquiries/edit/<int:enquiry_id>` | `edit` | `enquiries/edit.html` | Yes | Edit enquiry — rejects a mobile change onto another person's number (ADR-58) |
| GET/POST | `/enquiries/re-enquire/<int:enquiry_id>` | `re_enquire` | `enquiries/edit.html` (`reenquiry=True`) | Yes | "Log Another Enquiry" for someone already on file — updates their single enquiry row in place and re-opens it (`status='Interested'`); mobile is read-only (ADR-58) |
...
As of 2026-08-28 (ADR-58), a phone number identifies one person per admin: `_find_person_by_mobile()` is checked before every enquiry/student insert, `add()` never creates a second record for a known number, and `re_enquire()` is the path for a repeat visit (updates the existing row). `enquiry_id` is assigned via `database.id_sequence.insert_with_next_id()` (retries the `MAX+1` on a `23505` collision — TD-78). As of 2026-07-23 (ADR-18), `enquiries` reads/writes go through `database.supabase_client.get_supabase_client()` (Supabase is the source of truth), and as of 2026-07-24 (ADR-30) the only store — `add()`/`edit()`/`delete()`'s SQLite `INSERT`/`UPDATE`/`DELETE` calls were deleted outright, no SQLite dependency left anywhere in this file. As of 2026-07-23 (ADR-23), `index()`'s/`view()`'s `enquiry_id → student_id` lookup queries Supabase `students` directly. `_sanitize_date()` normalizes `followup_date` to `None` on blank/unparsable input before any Supabase call, since Postgres's `DATE` column is strictly typed unlike SQLite's. TD-36 (`admission()`'s `status='Admitted'` write not reaching Supabase) is now `Resolved` — see ADR-19.
...
| POST | `/memberships/refund-charge/<int:membership_id>/<charge_key>` | `refund_charge` | none — redirects to `student.view` | Yes | Refund a refundable membership charge (the security deposit): one Cashbook `Security Deposit Refund` expense + a `membership_charges.refunded_on` stamp; does not touch `total_fee`/`paid`/`pending` (ADR-66). Idempotent on `refunded_on` |
...
**As of 2026-09-02 (ADR-65/66)**, `create()`/`renew()` also read `database.shift_slots_queries.get_shift_slots()` and `database.membership_charges_queries.get_charge_config()`. When a `shift_slot_id` is posted, `total_fee`'s base becomes `compute_slot_charge(slot, plan, night_hours)` (`slot.monthly_fee × PLAN_MONTHS[plan]`, or `night_hourly_rate × hours`) instead of the flat plan fee; `plan_name` is then a term multiplier. Every compulsory charge plus every posted `applied_charge` is added (`_resolve_applied_charges()`), written as `membership_charges` rows (except `registration`, which stays `admission_fee_amount`), and the initial payment is split across all buckets by `split_payment_across_buckets()`. `ChargeTableUnavailable` (missing table + a real non-zero charge) makes `create()` delete the just-inserted membership and re-render. `renew()` re-charges only the recurring charges (seat/locker) and pre-selects the previous membership's slot. A Custom plan applies no slot fee and no extra charges (TD-92).
...
| Method | Path | Function | Template | Auth | Purpose |
|---|---|---|---|---|---|
| GET | `/settings/` | `index` | `settings/index.html` | Yes | Settings landing page (8 cards) |
| GET/POST | `/settings/membership` | `membership_settings` | `settings/membership_settings.html` | Yes | View/update membership plan fees, durations, fine rules, and (ADR-66) the extra-charge amounts + Compulsory/Optional flags; the `admission_fee` field is labelled "Registration Fee". Reminder-day inputs are a read-only summary from Notification Settings |
| GET/POST | `/settings/shift-slots` | `shift_slots` | `settings/shift_slots.html` | Yes | List + add time-window shift slots (ADR-65); catches `APIError` to flash "table missing" instead of 500 |
| POST | `/settings/shift-slots/<int:slot_id>/edit` | `shift_slot_edit` | none — redirects | Yes | Update one admin-scoped slot |
| POST | `/settings/shift-slots/<int:slot_id>/toggle` | `shift_slot_toggle` | none — redirects | Yes | Soft enable/disable a slot (never deleted) |
| GET/POST | `/settings/library` | `library_profile` | `settings/library_profile.html` | Yes | View/update library profile + logo/stamp/signature uploads; Seating Capacity now includes `night_capacity` (ADR-67) |
...
| POST | `/settings/backup/create` | `backup_create` | file download | Yes | Exports this admin's own rows across all 16 tenant-owned Supabase tables (ADR-75 gave `memberships`/`payments`/`membership_charges`/`panda_conversations`/`panda_messages`/`ai_center_settings`/`backup_log`/`shift_slots`/`expenses` a direct `admin_id` column, so `_collect_admin_backup_data()` no longer needs a join through `student_id` for any of them) as JSON, built and streamed entirely from memory (no local file written — works on a read-only-filesystem host), named `library_backup_<admin_id>_<timestamp>.json`, records it in `backup_log`, then serves it as a download |
| POST | `/settings/backup/archive` | `backup_archive` | redirect | Yes + rate-limited (2/hour) | **Danger Zone (ADR-76).** Deactivates this admin's account via `rpc_archive_tenant()` (`admins.status='archived'`) — requires a backup within the last 24h, a confirmation checkbox, the admin's own username typed back, and their current password. Clears the session; login is refused afterward |
| POST | `/settings/backup/delete` | `backup_delete` | redirect | Yes + rate-limited (2/hour) | **Danger Zone (ADR-76).** Permanently deletes every row this admin owns across all 16 tables plus the `admins` row itself, in one atomic transaction, via `rpc_delete_tenant_data()` — same confirmation requirements as Archive. Irreversible |
...
`data_backup`/`backup_export_csv`/`backup_create`/`backup_archive`/`backup_delete`: `data_backup` reads `get_backup_info(admin_id)` (Supabase, ADR-24) for the "last backup" display, and also passes `danger_zone_ready`/`danger_zone_window_hours`/`username` to the template so the Danger Zone card can decide whether to show its confirmation form or a "get a fresh export first" warning. `backup_export_csv` writes an admin-scoped `students` export via `csv.writer` into an in-memory `io.StringIO` — the `students` rows come from Supabase (`database.supabase_client.get_supabase_client()`). `backup_create` (labeled "Create Backup" in the UI) calls `_collect_admin_backup_data(admin_id)`, which queries all 16 tenant-owned Supabase tables filtered by `admin_id` directly (one uniform loop, `_BACKUP_DIRECT_TABLES` — see ADR-76), each wrapped in its own `try/except APIError` so a not-yet-created table on an older un-migrated project degrades to `[]` instead of failing the whole export; serializes the result to JSON built and streamed entirely from memory (`io.BytesIO`, no local file), calls `record_backup()` (Supabase, ADR-24), then serves it as a download. No scheduled/automatic backup exists (PF-5).

`backup_archive`/`backup_delete` (the "Danger Zone", ADR-76): both share `_danger_zone_confirmation_error(admin_id, form)`, which requires, in order: a backup recorded within the last `_DANGER_ZONE_BACKUP_WINDOW_HOURS` (24) hours (`_backup_is_fresh()`), a `confirm_downloaded` checkbox, `confirm_username` matching `session["username"]` exactly, and `current_password` verified against the stored hash (`check_password_hash`) — any failure flashes a specific message and redirects back to `/settings/backup` without calling either RPC. `backup_archive` then calls `rpc_archive_tenant()` (sets `admins.status='archived'`/`archived_at`, touches nothing else); `backup_delete` calls `rpc_delete_tenant_data()` (deletes every row the caller owns across all 16 tables plus `admins` itself, in one transaction, in FK-safe order — see ADR-76). Both log the action via `current_app.logger.info` (not a new DB table — the delete path may itself remove `audit_log`), clear the session, and redirect to `/` with a flash message. Both RPCs are parameterless — they derive the target tenant from `current_admin_id()` (the caller's own JWT claim), so no route or client input can direct either action at a different account.
