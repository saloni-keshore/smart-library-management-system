...
`admission()` guards against double-admission (same mobile+admin_id), copies `purpose`/`preferred_shift` from the enquiry, marks the enquiry `status='Admitted'`, and redirects to `membership.create` to force a membership to be created immediately after. As of 2026-07-23 (ADR-19), `students` reads/writes go through `database.supabase_client.get_supabase_client()` (Supabase is the source of truth for `index()`/`admission()`/`view()`/`edit()`), no query module — was raw SQL via `database.db.get_connection()` until this cutover. `admission()`/`edit()` also write a SQLite mirror (`database.db.get_connection()`) so `routes/payment.py`/`routes/dashboard.py`/`routes/membership_distribution.py`/`routes/notification.py`/etc. (all unmigrated) keep working against a real SQLite FK and their own raw `JOIN students` queries — `routes/membership.py` itself migrated to Supabase the same day (ADR-20) but still reads this same SQLite `students` mirror for its own `create()`/`renew()` student lookups, unaffected by that; `index()`/`view()` additionally use `get_connection()` read-only for `memberships`/`payments` — `memberships` is now Supabase-backed too (ADR-20) but this particular read stays against the SQLite mirror, which is kept current by `routes/membership.py`. `admission()` also reads/writes `enquiries` via `get_supabase_client()` (the same Supabase copy `routes/enquiries.py` owns) instead of the old SQLite-only read/update — its `status='Admitted'` flip now reaches Supabase directly, closing TD-36. `_sanitize_date()` normalizes `join_date` the same way `routes/enquiries.py`'s does for `followup_date`. Cross-blueprint links via `url_for` to `enquiry.index` and `membership.create`.
