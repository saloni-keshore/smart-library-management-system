...
As of 2026-07-23 (ADR-18), `enquiries` reads/writes go through `database.supabase_client.get_supabase_client()` (Supabase is the source of truth), no query module — was raw SQL via `database.db.get_connection()` until this cutover. `add()`/`edit()`/`delete()` also write a SQLite mirror (`database.db.get_connection()`) — as of 2026-07-24 (ADR-29), `routes/student.py`'s `admission()` no longer writes SQLite `students` at all, so this mirror's last FK dependent is gone and it's the **next removal candidate** in Phase 10 (see `docs/MIRROR_TRACKER.md`). As of 2026-07-23 (ADR-23), `index()`'s/`view()`'s `enquiry_id → student_id` lookup also queries Supabase `students` directly instead of the SQLite mirror — `add()`/`edit()`/`delete()`'s mirror-writes are the only SQLite access left in this file. `_sanitize_date()` normalizes `followup_date` to `None` on blank/unparsable input before any Supabase call, since Postgres's `DATE` column is strictly typed unlike SQLite's. TD-36 (`admission()`'s `status='Admitted'` write not reaching Supabase) is now `Resolved` — see ADR-19.
