...
3. For data access: if a `database/*_queries.py` module already exists for this feature (cashbook, BI, settings, membership settings), add a function there instead of writing raw SQL in the route — that's the pattern the more recently built features (Cashbook, BI, Settings) follow. Older features (auth, dashboard, students, enquiries, notifications) still use raw SQL inline; don't feel obligated to match that older style for new code.
