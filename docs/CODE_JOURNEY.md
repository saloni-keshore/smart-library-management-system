...
**Note (as of 2026-07-23, ADR-20):** step 2's `memberships` lookup and step 4's `paid_amount`/`pending_amount` update both still target **SQLite** — `routes/payment.py` is unmigrated. `routes/membership.py`'s `create()`/`renew()` (the only writers Supabase ever sees for this table) keep a SQLite mirror in sync for exactly this route's benefit, but this route's own write only reaches that mirror, not Supabase — so the Memberships page (Supabase-backed) shows a stale `paid_amount`/`pending_amount` immediately after this flow runs, until the same membership is next touched by `create()`/`renew()`. See TD-37.
