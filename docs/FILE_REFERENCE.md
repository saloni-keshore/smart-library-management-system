...
- **Depended on by:** `database/audit_queries.py`, `database/bi_queries.py`, `database/cashbook_queries.py`, `database/membership_settings_queries.py`, `database/settings_queries.py`, `utils/charts.py`, `routes/dashboard.py`, `routes/enquiries.py`, `routes/membership.py`, `routes/membership_distribution.py`, `routes/notification.py`, `routes/payment.py`, `routes/student.py`, `routes/setting.py` (all routes except `security_settings()`'s password branch — see below). Also imported by every `migrate_*.py` script and `database/migrate.py`, but via the bare `from db import get_connection` form rather than `from database.db import get_connection` — an inconsistent import style that only works because those scripts are always run standalone (Known Technical Debt item TD-21). `routes/auth.py`'s `login()`/`forgot_password()` no longer use this as of 2026-07-23 (ADR-16) — but `register()` still does, as a one-table mirror-insert bridge (see its own card below, and TD-35). `routes/setting.py`'s `security_settings()` password-change branch also stopped using this as of 2026-07-23 (ADR-17) — every other function in that file still does.
...
- **Depended on by:** `routes/auth.py` (as of 2026-07-23 — see ADR-16); `routes/setting.py`'s `security_settings()` password-change branch only (as of 2026-07-23 — see ADR-17); `tests/conftest.py`'s `get_admin_by_username()` helper and `tests/test_01_auth.py`.
- **Future modification notes:** This is the first reusable app-facing Supabase client — as more modules are cut over from SQLite (per the incremental migration plan in ADR-16), they should import `get_supabase_client()` from here rather than each rolling their own `create_client(...)` call. `routes/setting.py`'s `security_settings()` is the second consumer (ADR-17), reusing the same cached instance. `database/migrate_to_supabase.py` deliberately keeps its own separate `get_supabase_client()` (it's a standalone script, not part of the running app) — don't merge the two.
