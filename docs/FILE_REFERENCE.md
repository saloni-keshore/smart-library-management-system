...
- **Functions/Classes:** `generate_receipt_number(conn, admin_id)`, `record_payment(conn, admin_id, membership_id, student_id, student_name, payment_mode, amount, remarks, category, description, source)`.
...
- **Depends on:** `database.db.get_connection`, `database.membership_queries.EFFECTIVE_STATUS_SQL` (added 2026-07-21).
...
- **Future modification notes:** `analytics.html` is rendered with zero context (Known Technical Debt item TD-8) — don't assume it has real data if you're extending it. `index.html`, `create.html`, and `renew.html` all gained a `get_flashed_messages()` block (fixed 2026-07-22 — TD-31) — none rendered flashes before, so every validation/success/rollback-error message `create()`/`renew()` set (including the negative-amount and DB-error checks added the same day, see [CHANGELOG.md](CHANGELOG.md)) was silently discarded; `distribution.html`/`analytics.html` still don't render flashes (out of this audit's Payment-workflow scope).
